	...
	public async generate() {
		...
		for (const classDescription of result) {
			try {
				...
				const dataClassGenerator = GeneratorFactory.getInstance(classDescription.language);
				...
				// Create a new File 
				await fs.writeFile(path.resolve(classDescription.classFilePath) + `/${classDescription.className}.${fileExtension}`, generatedClassCode);
...
const dataClassGenerator = new Main();
